// ============================================================================
// tone_test.v
// Tone stack frequency test — generates white-ish noise or sine sweep
// through the tone stack, outputs to DAC. Listen for EQ effect.
//
// S1 = cycle tone preset (0-4): flat, Fender, Marshall, Vox, Metal
// S2 = toggle source: sine 440Hz vs pseudo-random noise
//
// LED[0] = heartbeat
// LED[1] = source (off=sine, on=noise)
// LED[2:4] = tone preset (binary)
// LED[5] = output activity
// ============================================================================
...
always @(posedge clk_27m or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (!btn_s1 && s1_prev && s1_db == 0) begin
            preset_sel <= (preset_sel >= 3'd4) ? 3'd0 : preset_sel + 1;
            s1_db <= 16'hFFFF;
        end
...
always @(posedge clk_27m or negedge rst_n) begin
    if (!rst_n) begin
        ...
        sine_out <= sine_table[sine_idx];
        sine_idx <= (sine_idx >= 7'd107) ? 7'd0 : sine_idx + 7'd1;
    end
end

// ── LFSR noise generator (16-bit, taps at 16,15,13,4) ───────────────────
reg [15:0] lfsr;
reg signed [31:0] noise_out;
...
always @(posedge clk_27m or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // Scale noise to similar level as sine (~0.5V in Q16.16)
        noise_out <= {{16{lfsr[15]}}, lfsr};
    end
end

// ── Source select ────────────────────────────────────────────────────────
wire signed [31:0] source = noise_mode ? noise_out : sine_out;
...
reg out_act;
always @(posedge clk_27m or negedge rst_n) begin
    if (!rst_n) begin out_cnt <= 0; out_act <= 0; end
    else begin
        if (tone_valid) begin out_act <= 1; out_cnt <= 0; end
        else if (out_cnt < 20'd675_000) out_cnt <= out_cnt + 1;
        else out_act <= 0;
    end
end
...
// Active-low LEDs
assign led[0] = ~hb;
assign led[1] = ~noise_mode;
assign led[2] = ~preset_sel[0];
assign led[3] = ~preset_sel[1];
assign led[4] = ~preset_sel[2];
assign led[5] = ~out_act;
